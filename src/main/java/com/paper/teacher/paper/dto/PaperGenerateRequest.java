...
public record PaperGenerateRequest(
        ...
        @NotNull(message = "组卷范围类型不能为空") PaperScopeType scopeType,
    ...
    public boolean isChapterScopeValid() {
        return scopeType != PaperScopeType.CHAPTERS || (chapters != null && !chapters.isEmpty());
    }

    @AssertTrue(message = "UNITS 范围必须至少选择一个单元")
    public boolean isUnitScopeValid() {
        return scopeType != PaperScopeType.UNITS || (units != null && !units.isEmpty());
    }
